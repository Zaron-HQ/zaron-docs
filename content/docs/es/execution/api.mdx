...
Zaron proporciona una API externa completa para consultar registros de ejecución de flujos de trabajo y configurar webhooks para notificaciones en tiempo real cuando los flujos de trabajo se completan.
...
Puedes generar claves de API desde la configuración de usuario en el panel de control de Zaron.
...
Cuando se completa la ejecución de un flujo de trabajo, Zaron envía la siguiente carga útil (vía webhook POST, correo electrónico o Slack):
...
- `Zaron-event`: Tipo de evento (siempre `workflow.execution.completed`)
- `Zaron-timestamp`: Marca de tiempo Unix en milisegundos
- `Zaron-delivery-id`: ID único de entrega para idempotencia
- `Zaron-signature`: Firma HMAC-SHA256 para verificación (si se configura un secreto)
- `Idempotency-Key`: Igual que el ID de entrega para detección de duplicados

### Verificación de firma

Si configuras un secreto de webhook, verifica la firma para asegurar que el webhook proviene de Zaron:
...
2. **Seguridad de webhook**: Siempre configura un secreto de webhook y verifica las firmas para asegurar que las solicitudes provienen de Zaron.
