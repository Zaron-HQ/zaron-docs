...
Copilot es tu asistente integrado en el editor que te ayuda a crear y editar flujos de trabajo con Zaron Copilot, así como a entenderlos y mejorarlos. Puede:
...
<Callout type="info">
  Copilot es un servicio gestionado por Zaron. Para implementaciones autoalojadas, genera una clave API de Copilot en la aplicación alojada (zaron.dev → Configuración → Copilot)
  1. Ve a [zaron.dev](https://zaron.dev) → Configuración → Copilot y genera una clave API de Copilot
...
Puedes usar Copilot como servidor MCP en tu editor o cliente de IA favorito. Esto te permite construir, probar, desplegar y gestionar flujos de trabajo de Zaron directamente desde herramientas como Cursor, Claude Code, Claude Desktop y VS Code.
...
La clave se verá como `sk-Zaron-copilot-...`. Usarás esto en la configuración a continuación.
