...
Zaron fournit une API externe complète pour interroger les journaux d'exécution des workflows et configurer des webhooks pour des notifications en temps réel lorsque les workflows sont terminés.
...
Vous pouvez générer des clés API depuis vos paramètres utilisateur dans le tableau de bord Zaron.
...
Lorsqu'une exécution de workflow se termine, Zaron envoie la charge utile suivante (via webhook POST, e-mail ou Slack) :
...
- `Zaron-event` : type d'événement (toujours `workflow.execution.completed`)
- `Zaron-timestamp` : horodatage Unix en millisecondes
- `Zaron-delivery-id` : ID de livraison unique pour l'idempotence
- `Zaron-signature` : signature HMAC-SHA256 pour vérification (si un secret est configuré)
- `Idempotency-Key` : identique à l'ID de livraison pour la détection des doublons

### Vérification de signature

Si vous configurez un secret webhook, vérifiez la signature pour vous assurer que le webhook provient de Zaron :
...
2. **Sécurité des webhooks** : Configurez toujours un secret de webhook et vérifiez les signatures pour vous assurer que les requêtes proviennent de Zaron.
