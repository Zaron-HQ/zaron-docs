...
- Partagé via un lien direct (par exemple, `https://zaron.dev/form/my-survey`)
...
3. Configurez :
   - **URL** : identifiant unique (par exemple, `contact-form` → `zaron.dev/form/contact-form`)
...
```
https://zaron.dev/form/your-identifier
```
...
```html
<iframe
  src="https://zaron.dev/form/your-identifier"
  width="100%"
  height="600"
  frameborder="0"
  title="Form"
...
```bash
curl -X POST https://zaron.dev/api/form/your-identifier \
...
```typescript
const response = await fetch('https://zaron.dev/api/form/your-identifier', {
  method: 'POST',
...
**Le formulaire ne se charge pas dans l'iframe** - Vérifiez que la CSP de votre site autorise les iframes provenant de `zaron.dev`.
