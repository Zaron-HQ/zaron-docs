...
Copilot est votre assistant intégré à l'éditeur qui vous aide à créer et à modifier des flux de travail avec Zaron Copilot, ainsi qu'à les comprendre et à les améliorer. Il peut :
...
<Callout type="info">
  Copilot est un service géré par Zaron. Pour les déploiements auto-hébergés, générez une clé API Copilot dans l'application hébergée (zaron.dev → Paramètres → Copilot)
  1. Allez sur [zaron.dev](https://zaron.dev) → Paramètres → Copilot et générez une clé API Copilot
...
Vous pouvez utiliser Copilot comme serveur MCP dans votre éditeur ou client IA préféré. Cela vous permet de créer, tester, déployer et gérer des workflows Zaron directement depuis des outils comme Cursor, Claude Code, Claude Desktop et VS Code.
...
La clé ressemblera à `sk-Zaron-copilot-...`. Vous l'utiliserez dans la configuration ci-dessous.
