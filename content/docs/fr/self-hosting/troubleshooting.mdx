...
docker compose ps zaronsdk
docker compose logs zaronsdk
...
# Run manually
docker compose exec zaronsdk bun run db:migrate
```
...
docker compose exec zaronsdk apt-get update && apt-get install -y ca-certificates
...
# Specific service
docker compose logs -f zaronsdk
```

## Obtenir de l'aide

- [Problèmes GitHub](https://github.com/zaronai/Zaron/issues)
