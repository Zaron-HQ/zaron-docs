...
Copilot 是您编辑器中的助手，可帮助您使用 Zaron Copilot 构建和编辑工作流，同时理解和改进它们。它可以：
...
<Callout type="info">
  Copilot 是一个由 Zaron 管理的服务。对于自托管部署，请在托管应用程序中生成一个 Copilot API 密钥（zaron.dev → 设置 → Copilot）
  1. 前往 [zaron.dev](https://zaron.dev) → 设置 → Copilot 并生成一个 Copilot API 密钥
...
你可以在常用编辑器或 AI 客户端中将 Copilot 作为 MCP 服务器使用。这样可以直接在 Cursor、Claude Code、Claude Desktop 和 VS Code 等工具中构建、测试、部署和管理 Zaron 工作流。
...
密钥类似于 `sk-Zaron-copilot-...`。你将在下方配置中用到它。
