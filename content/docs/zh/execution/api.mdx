...
Zaron 提供了一个全面的外部 API，用于查询工作流执行日志，并在工作流完成时设置实时通知的 webhook。
...
当工作流执行完成时，Zaron 会发送以下负载（通过 webhook POST、电子邮件或 Slack）：
...
- `Zaron-event`：事件类型（始终为 `workflow.execution.completed`）
- `Zaron-timestamp`：以毫秒为单位的 Unix 时间戳
- `Zaron-delivery-id`：用于幂等性的唯一交付 ID
- `Zaron-signature`：用于验证的 HMAC-SHA256 签名（如果配置了密钥）
- `Idempotency-Key`：与交付 ID 相同，用于检测重复

### 签名验证

如果您配置了 webhook 密钥，请验证签名以确保 webhook 来自 Zaron：
...
2. **Webhook 安全性**：始终配置一个 webhook 密钥并验证签名，以确保请求来自 Zaron。
