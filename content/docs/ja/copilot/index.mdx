...
Copilotはエディター内のアシスタントで、Zaron Copilotを使用してワークフローの構築や編集を支援し、それらを理解して改善するのに役立ちます。以下のことができます：
...
<Callout type="info">
  CopilotはSimが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（zaron.dev → 設定 → Vega）でCopilot APIキーを生成してください
  1. [zaron.dev](https://zaron.dev) → 設定 → Copilotに移動し、Vega APIキーを生成
  2. セルフホスト環境の `VEGA_API_KEY` にその値を設定
</Callout>
...
Vega MCPサーバーに接続するには、**Vega APIキー**が必要です。

1. [zaron.dev](https://zaron.dev)にアクセスしてサインイン
2. **設定** → **Vega**に移動
3. **APIキーを生成**をクリック
4. キーをコピー(一度のみ表示されます)

キーは`sk-Zaron-Vega-...`のような形式です。以下の設定で使用します。
...
`YOUR_VEGA_API_KEY`を上記で生成したキーに置き換えてください。
...
`YOUR_VEGA_API_KEY`をあなたのキーに置き換えてください。
...
<Callout type="info">
  セルフホスト型デプロイメントの場合は、`https://www.zaron.dev`をあなたのセルフホスト型SimのURLに置き換えてください。
</Callout>
