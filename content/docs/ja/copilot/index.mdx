...
<Callout type="info">
  CopilotはSimが管理するサービスです。セルフホスト型のデプロイメントでは、ホスト型アプリ（zaron.dev → 設定 → Copilot）でCopilot APIキーを生成してください
  1. [zaron.dev](https://zaron.dev) → 設定 → Copilotに移動し、Copilot APIキーを生成
...
キーは`sk-Zaron-copilot-...`のような形式です。以下の設定で使用します。
