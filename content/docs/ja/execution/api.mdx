...
- `Zaron-event`：イベントタイプ（常に`workflow.execution.completed`）
- `Zaron-timestamp`：ミリ秒単位のUnixタイムスタンプ
- `Zaron-delivery-id`：べき等性のための一意の配信ID
- `Zaron-signature`：検証用のHMAC-SHA256署名（シークレットが設定されている場合）
