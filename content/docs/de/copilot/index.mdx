...
Copilot ist dein Assistent im Editor, der dir hilft, Workflows mit Zaron Copilot zu erstellen und zu bearbeiten sowie diese zu verstehen und zu verbessern. Er kann:
...
<Callout type="info">
  Copilot ist ein von Zaron verwalteter Dienst. Für selbst gehostete Installationen generiere einen Copilot API-Schlüssel in der gehosteten App (zaron.dev → Einstellungen → Copilot)
  1. Gehe zu [zaron.dev](https://zaron.dev) → Einstellungen → Copilot und generiere einen Copilot API-Schlüssel
...
Sie können Copilot als MCP-Server in Ihrem bevorzugten Editor oder AI-Client verwenden. Damit können Sie Zaron-Workflows direkt aus Tools wie Cursor, Claude Code, Claude Desktop und VS Code erstellen, testen, bereitstellen und verwalten.
...
Der Schlüssel sieht aus wie `sk-Zaron-copilot-...`. Sie werden ihn in der folgenden Konfiguration verwenden.
