...
Vega ist dein Assistent im Editor, der dir hilft, Workflows mit Zaron Vega zu erstellen und zu bearbeiten sowie diese zu verstehen und zu verbessern. Er kann:

- **Erklären**: Beantwortet Fragen zu Zaron und deinem aktuellen Workflow
...
<Callout type="info">
  Vega ist ein von Zaron verwalteter Dienst. Für selbst gehostete Installationen generiere einen Vega API-Schlüssel in der gehosteten App (zaron.dev → Einstellungen → Vega)
  1. Gehe zu [zaron.dev](https://zaron.dev) → Einstellungen → Vega und generiere einen Vega API-Schlüssel
  2. Setze `VEGA_API_KEY` in deiner selbst gehosteten Umgebung auf diesen Wert
</Callout>

## Kontextmenü (@)

Verwende das `@` Symbol, um auf verschiedene Ressourcen zu verweisen und Vega mehr Kontext über deinen Arbeitsbereich zu geben:

<Image
  src="/static/Vega/Vega-menu.png"
  alt="Vega-Kontextmenü mit verfügbaren Referenzoptionen"
  width={600}
  height={400}
/>

Das `@` Menü bietet Zugriff auf:
- **Chats**: Verweise auf vorherige Vega-Gespräche
...
- **Dokumentation**: Verweise auf Zaron-Dokumentation
- **Vorlagen**: Verweise auf Workflow-Vorlagen
- **Logs**: Verweise auf Ausführungsprotokolle und Ergebnisse

Diese kontextbezogenen Informationen helfen Vega, genauere und relevantere Unterstützung für deinen spezifischen Anwendungsfall zu bieten.
...
<Cards>
  ...
  >
    <div className="m-0 text-sm">
      Erstellen-und-Bearbeiten-Modus. Vega schlägt spezifische Änderungen vor (Blöcke hinzufügen, Variablen verbinden, Einstellungen anpassen) und wendet sie an, wenn du zustimmst.
    </div>
  </Card>
</Cards>
...
<div className="flex justify-center">
  <Image
    ...
    alt="Vega-Modusauswahl-Oberfläche"
    width={600}
    height={400}
    className="my-6"
  />
...
Du kannst einfach zwischen verschiedenen Denkmodi über die Modusauswahl in der Vega-Oberfläche wechseln:

<Image
  src="/static/Vega/Vega-models.png"
  alt="Vega-Modusauswahl zeigt den erweiterten Modus mit MAX-Umschalter"
  width={600}
  height={300}
/>
...
Die Vega-Nutzung wird pro Token vom zugrundeliegenden LLM abgerechnet:
...
Wenn du eine Nachricht sendest, während Vega noch antwortet, wird sie in die Warteschlange gestellt. Du kannst:
...
Wenn Vega Änderungen an deinem Workflow vornimmt, speichert es Checkpoints, damit du bei Bedarf zurückkehren kannst.

### Checkpoints anzeigen

Fahre mit der Maus über eine Vega-Nachricht und klicke auf das Checkpoints-Symbol, um gespeicherte Workflow-Zustände für diese Nachricht anzuzeigen.
...
Wenn Vega Änderungen vorschlägt, kannst du:
...
Bei komplexen Anfragen kann Vega seinen Denkprozess in erweiterbaren Denkblöcken anzeigen:

- Blöcke werden automatisch erweitert, während Vega denkt
- Klicken zum manuellen Erweitern/Reduzieren
- Zeigt die Dauer des Denkprozesses an
- Hilft dir zu verstehen, wie Vega zu seiner Lösung gekommen ist

## Optionsauswahl

Wenn Vega mehrere Optionen präsentiert, kannst du auswählen mit:
...
| `Esc` | Menüs schließen |
| `Mod+Shift+Enter` | Vega-Änderungen akzeptieren |

## Nutzungslimits

Die Vega-Nutzung wird pro Token des zugrunde liegenden LLM abgerechnet. Wenn Sie Ihr Nutzungslimit erreichen, fordert Vega Sie auf, Ihr Limit zu erhöhen. Sie können die Nutzung in Schritten (50 $, 100 $) von Ihrer aktuellen Basis aus hinzufügen.
...
Sie können Vega als MCP-Server in Ihrem bevorzugten Editor oder AI-Client verwenden. Damit können Sie Zaron-Workflows direkt aus Tools wie Cursor, Claude Code, Claude Desktop und VS Code erstellen, testen, bereitstellen und verwalten.

### Generieren eines Vega-API-Schlüssels

Um sich mit dem Vega-MCP-Server zu verbinden, benötigen Sie einen **Vega-API-Schlüssel**:

1. Gehen Sie zu [zaron.dev](https://zaron.dev) und melden Sie sich an
2. Navigieren Sie zu **Einstellungen** → **Vega**
3. Klicken Sie auf **API-Schlüssel generieren**
4. Kopieren Sie den Schlüssel – er wird nur einmal angezeigt

Der Schlüssel sieht aus wie `sk-Zaron-Vega-...`. Sie werden ihn in der folgenden Konfiguration verwenden.
...
Ersetzen Sie `YOUR_VEGA_API_KEY` durch den oben generierten Schlüssel.

### Claude Code

Führen Sie den folgenden Befehl aus, um den Vega MCP-Server hinzuzufügen:
...
Ersetzen Sie `YOUR_VEGA_API_KEY` durch Ihren Schlüssel.
...
<Callout type="info">
  Für selbst gehostete Deployments ersetzen Sie `https://www.zaron.dev` durch Ihre selbst gehostete Zaron-URL.
</Callout>
