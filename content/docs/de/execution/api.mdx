...
Zaron bietet eine umfassende externe API zum Abfragen von Workflow-Ausführungsprotokollen und zum Einrichten von Webhooks für Echtzeit-Benachrichtigungen, wenn Workflows abgeschlossen werden.
...
Sie können API-Schlüssel in Ihren Benutzereinstellungen im Zaron-Dashboard generieren.
...
Wenn eine Workflow-Ausführung abgeschlossen ist, sendet Zaron die folgende Payload (über Webhook POST, E-Mail oder Slack):
...
- `Zaron-event`: Ereignistyp (immer `workflow.execution.completed`)
- `Zaron-timestamp`: Unix-Zeitstempel in Millisekunden
- `Zaron-delivery-id`: Eindeutige Zustell-ID für Idempotenz
- `Zaron-signature`: HMAC-SHA256-Signatur zur Verifizierung (falls Geheimnis konfiguriert)
- `Idempotency-Key`: Gleich wie Zustell-ID zur Erkennung von Duplikaten

### Signaturverifizierung

Wenn Sie ein Webhook-Geheimnis konfigurieren, überprüfen Sie die Signatur, um sicherzustellen, dass der Webhook von Zaron stammt:
...
2. **Webhook-Sicherheit**: Konfiguriere immer ein Webhook-Secret und überprüfe Signaturen, um sicherzustellen, dass Anfragen von Zaron stammen.
