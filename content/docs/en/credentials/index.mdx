...
Zaron supports two categories of credentials: **secrets** for static values like API keys, and **OAuth accounts** for authenticated service connections.
...
OAuth accounts are authenticated connections to third-party services. Zaron handles the OAuth flow, token storage, and automatic refresh. You can connect **multiple accounts per provider** — for example, two separate Gmail accounts for different workflows.

Zaron uses two OAuth tiers depending on the provider:

### Tier 1 — Native Connections (Google, Slack, Microsoft, and more)

For the most common enterprise services, Zaron manages the OAuth application directly. No configuration required — just click **Connect** and authorize.

Tier 1 providers include:
- **Google** — Gmail, Drive, Docs, Sheets, Forms, Calendar, Contacts, BigQuery, Ads, Tasks, Meet, Groups, Vault, Vertex AI
- **Microsoft** — Outlook, Teams, OneDrive, SharePoint, Excel, Planner, Azure AD, Dataverse
- **Atlassian** — Confluence, Jira
- **Salesforce**
- **HubSpot**
- **Slack**

### Tier 2 — Composio (250+ providers)

All other services — Notion, Linear, Airtable, Asana, Zoom, LinkedIn, Discord, Telegram, Webflow, Trello, Shopify, Spotify, and hundreds more — connect through **Composio**, Zaron's integration layer.

Composio connections appear in your credential selector exactly like native connections. The workflow execution engine routes them automatically.

<Callout type="info">
Orbit (the Zaron AI assistant) is aware of both Tier 1 and Tier 2 providers. When you ask Orbit to use a service that requires OAuth, it will check for an existing connection and prompt you to connect if one is not found.
</Callout>
...
When a workflow runs, Zaron resolves secrets in this order:
...
<FAQ items={[
  ...
  { question: "What is the difference between Tier 1 and Tier 2 (Composio) OAuth?", answer: "Tier 1 providers (Google, Slack, Microsoft, Salesforce, HubSpot, Atlassian) connect using Zaron's own OAuth applications — no setup needed, they just work for all users. Tier 2 providers (Notion, Linear, Airtable, Zoom, Discord, and 250+ more) connect through Composio, Zaron's integration layer. From your perspective as a user the experience is identical — pick a provider, authorize it, and it appears in your credential selector." },
