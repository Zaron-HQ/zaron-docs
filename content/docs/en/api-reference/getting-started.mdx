...
Go to the Zaron platform and navigate to **Settings**, then go to **Zaron Keys** and click **Create**. See [Authentication](/api-reference/authentication) for details on key types.
</Step>

<Step>
### Find your workflow ID

Open a workflow in the Zaron editor. The workflow ID is in the URL:

```
https://www.zaron.dev/workspace/{workspaceId}/w/{workflowId}
```
...
<Tabs items={['curl', 'TypeScript', 'Python']}>
  <Tab value="curl">
    ```bash
    curl -X POST https://www.zaron.dev/api/workflows/{workflowId}/execute \
  ...
  <Tab value="TypeScript">
    ...
    const response = await fetch(
      `https://www.zaron.dev/api/workflows/${workflowId}/execute`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-API-Key': process.env.ZARON_API_KEY!,
        },
  ...
  <Tab value="Python">
    ...
    response = requests.post(
        f"https://www.zaron.dev/api/workflows/{workflow_id}/execute",
        headers={
            "Content-Type": "application/json",
            "X-API-Key": os.environ["ZARON_API_KEY"],
        },
        json={"inputs": {}},
    )
...
```bash
curl https://www.zaron.dev/api/jobs/{taskId} \
  -H "X-API-Key: YOUR_API_KEY"
```
...
```bash
# First page
curl "https://www.zaron.dev/api/v1/logs?limit=20" \
  -H "X-API-Key: YOUR_API_KEY"

# Next page — use the nextCursor from the previous response
curl "https://www.zaron.dev/api/v1/logs?limit=20&cursor=abc123" \
  -H "X-API-Key: YOUR_API_KEY"
```
