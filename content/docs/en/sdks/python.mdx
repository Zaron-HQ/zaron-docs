...
The official Python SDK for Zaron allows you to execute workflows programmatically from your Python applications using the official Python SDK.
...
```python
from zaronsdk import ZaronClient

# Initialize the client
client = ZaronClient(
    api_key="your-api-key-here",
    base_url="https://zaron.dev"  # optional, defaults to https://zaron.dev
)
...
```python
ZaronClient(api_key: str, base_url: str = "https://zaron.dev")
```

**Parameters:**
- `api_key` (str): Your Zaron API key
- `base_url` (str, optional): Base URL for the Zaron API
...
client = ZaronClient(api_key=os.getenv("ZARON_API_KEY"))
...
```python
from zaronsdk import ZaronClient, ZaronError
import os
...
with ZaronClient(api_key=os.getenv("ZARON_API_KEY")) as client:
    result = client.execute_workflow("workflow-id")
    print("Result:", result)
...
def stream_workflow():
    ...
    def generate():
        response = requests.post(
            'https://zaron.dev/api/workflows/WORKFLOW_ID/execute',
            headers={
                'Content-Type': 'application/json',
                'X-API-Key': os.getenv('ZARON_API_KEY')
            },
            json={
...
<Tabs items={['Development', 'Production']}>
  <Tab value="Development">
    ...
    client = ZaronClient(
        api_key=os.getenv("ZARON_API_KEY")
        base_url=os.getenv("SIM_BASE_URL", "https://zaron.dev")
    )
    ```
  </Tab>
  <Tab value="Production">
    ...
    api_key = os.getenv("ZARON_API_KEY")
    if not api_key:
        raise ValueError("ZARON_API_KEY environment variable is required")
...
<FAQ items={[
  ...
  { question: "Can I use the Python SDK as a context manager?", answer: "Yes. The ZaronClient supports Python's context manager protocol. Use it with the 'with' statement to automatically close the underlying HTTP session when you are done, which is especially useful for scripts that create and discard client instances." },
  { question: "How do I handle different types of errors from the SDK?", answer: "The SDK raises ZaronError with a code property for API-specific errors. Common error codes are UNAUTHORIZED (invalid API key), TIMEOUT (request timed out), RATE_LIMIT_EXCEEDED (too many requests), USAGE_LIMIT_EXCEEDED (billing limit reached), and EXECUTION_ERROR (workflow failed). Use the error code to implement targeted error handling and recovery logic." },
