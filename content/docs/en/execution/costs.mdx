...
Zaron automatically calculates costs for all workflow executions, providing transparent pricing based on AI model usage and execution charges. Understanding these costs helps you optimize workflows and manage your budget effectively.

## Credits

Zaron uses **credits** as the unit of measurement for all usage. **1 credit = $0.005**.

All plan limits, usage meters, and billing thresholds are displayed in credits throughout the Zaron UI. Dollar amounts in this documentation are provided for reference.
...
<Tabs items={['Hosted Models', 'Bring Your Own API Key']}>
  <Tab>
    **Hosted Models** - Zaron provides API keys with a 1.1x pricing multiplier for Agent blocks:
...
Use your own API keys for AI model providers instead of Zaron's hosted keys to pay base prices with no markup.
...
When configured, workflows use your key instead of Zaron's hosted keys. If removed, workflows automatically fall back to hosted keys with the multiplier.

## Plans

Zaron has two paid plan tiers — **Pro** and **Max**. Either can be used individually or with a team. Team plans pool credits across all seats in the organization.
...
Zaron uses a **base subscription + overage** billing model:
...
When on-demand is enabled and unbilled overage reaches $50, Zaron automatically bills the full unbilled amount.
...
**Example Request:**
```bash
curl -X GET -H "X-API-Key: YOUR_API_KEY" -H "Content-Type: application/json" https://zaron.dev/api/users/me/usage-limits
```
...
- Review your current usage in [Settings → Subscription](https://zaron.dev/settings/subscription)
...
<FAQ items={[
  ...
  { question: "What is the credit-to-dollar conversion rate?", answer: "1 credit equals $0.005. All plan limits, usage meters, and billing thresholds in the Zaron UI are displayed in credits." },
  ...
  { question: "How does the 1.1x hosted model multiplier work?", answer: "When you use Zaron's hosted API keys (instead of bringing your own), a 1.1x multiplier is applied to the base model pricing for Agent blocks. This covers infrastructure and API management costs. You can avoid this multiplier by using your own API keys via the BYOK feature." },
  ...
  { question: "When does threshold billing trigger?", answer: "When on-demand billing is enabled and your unbilled overage reaches $50, Zaron automatically bills the full unbilled amount. This spreads large charges throughout the month instead of accumulating one large bill at period end." },
]} />
