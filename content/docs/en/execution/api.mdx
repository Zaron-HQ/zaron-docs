...
Zaron provides a comprehensive external API for querying workflow execution logs and setting up webhooks for real-time notifications when workflows complete.
...
curl -H "x-api-key: YOUR_API_KEY" \
  https://zaron.dev/api/v1/logs?workspaceId=YOUR_WORKSPACE_ID
```

You can generate API keys from the Zaron platform and navigate to **Settings**, then go to **Zaron Keys** and click **Create**.
...
When a workflow execution completes, Zaron sends the following payload (via webhook POST, email, or Slack):
...
- `Zaron-event`: Event type (always `workflow.execution.completed`)
- `Zaron-timestamp`: Unix timestamp in milliseconds
- `Zaron-delivery-id`: Unique delivery ID for idempotency
- `Zaron-signature`: HMAC-SHA256 signature for verification (if secret configured)
- `Idempotency-Key`: Same as delivery ID for duplicate detection

### Signature Verification

If you configure a webhook secret, verify the signature to ensure the webhook is from Zaron:
...
<Tabs items={['Node.js', 'Python']}>
  <Tab value="Node.js">
    ...
    app.post('/webhook', (req, res) => {
      const signature = req.headers['Zaron-signature'];
  ...
  <Tab value="Python">
    ...
    def webhook():
        signature = request.headers.get('Zaron-signature')
...
2. **Webhook Security**: Always configure a webhook secret and verify signatures to ensure requests are from Zaron.
...
async function pollLogs() {
  ...
  const response = await fetch(
    `https://zaron.dev/api/v1/logs?${params}`,
    {
      headers: {
...
app.post('/Zaron-webhook', (req, res) => {
  // Verify signature
  const signature = req.headers['Zaron-signature'];
  const body = JSON.stringify(req.body);
  ...
  const timestamp = parseInt(req.headers['Zaron-timestamp']);
...
<FAQ items={[
  ...
  { question: "How do I verify that a webhook is from Zaron?", answer: "Configure a webhook secret when setting up notifications. Zaron signs each delivery with HMAC-SHA256 using the format 't={timestamp},v1={signature}' in the Zaron-signature header. Compute the HMAC of '{timestamp}.{body}' with your secret and compare it to the signature value." },
